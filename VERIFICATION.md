...
Gate: bun run verify exit 0 — 949 tests across 51 files, both typechecks, the production build,
      four audits. 940 → 949 tests; the 9 added are this loop's.
```

Iteration 1 built §4.11 stage 1 only: the envelope, the five types, directory-backed persistence
and the synchronous-mutation invariant. Stages 2–10 are untouched, so AS-003…AS-016 are NOT TESTED
rather than FAIL — nothing has been attempted for them.
...
### AS-003…AS-016 — **NOT TESTED**

Nothing has been attempted. Stage 1 of the §4.11 build order is done; stages 2–10 are not started.
Per-item status is unchanged from iteration 0.
